...
const ConfidentialityAgreementPdf = ({ data }) => {
  const { name = '', position = '', date = '', signature } = data;
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.docCode}>FM-03 | ვ.2 | 2026 | ISO/IEC 17020:2012/2013 §5</Text>
        ...
        <Text style={{ fontSize: 7, color: '#aaa', textAlign: 'center' }}>
          კონფიდენციალური — მხოლოდ შიდა გამოყენებისთვის | FM-03 v.2 | შპს „ბილდექს ექსპერტიზა"
        </Text>
      </Page>
    </Document>
  );
