    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.body}>
          ...
          <Text style={s.paraIn}>{`(ე) გადაცემული წილი სრულად განთავსებულ კაპიტალს წარმოადგენს; არ არსებობს დაუფარავი შენატანის ვალდებულება ან საზოგადოების წინაშე სხვა გადაუხდელი ვალი, რომელიც შეიძლება გადაეცეს წილის შემძენს;`}</Text>
          ...
          <Text style={s.para}>{`${art(2)}.2. წილის შემძენი ადასტურებს და გარანტიას იძლევა, რომ:`}</Text>
          ...
          <Text style={s.paraIn}>{`(ბ) გადასცეს წილის შემძენს ხელშეკრულების ხელმოწერილი ეგზემპლარი, შესაბამისი კორპორაციული გადაწყვეტილება წილის გადაცემის შესახებ და წილის სტატუსთან დაკავშირებული ყველა სხვა სათანადო დოკუმენტი;`}</Text>
          ...
          <Text style={s.paraIn}>{`(ე) სრულად ითანამშრომლოს წილის შემძენთან და სსიპ საჯარო რეესტრის ეროვნულ სააგენტოსთან სახელმწიფო რეგისტრაციის ოპერატიულად განხორციელებისათვის; ნებისმიერი ხარვეზის ან მოთხოვნის შემთხვევაში ოპერატიულად წარადგინოს დამატებითი დოკუმენტები;`}</Text>
          <Text style={s.paraIn}>{isGift
            ...
            : `(ვ) გადასცეს წილის შემძენს ბოლო 3 (სამი) საფინანსო წლის ანგარიშგება (ან ყველა ხელმისაწვდომი ანგარიშგება, თუ საზოგადოება 3 წელზე ნაკლები ხნის წინ დაფუძნდა) და ნებისმიერი სხვა ინფორმაცია, რომელიც წილის შემძენს ესაჭიროება.`}
          </Text>
          ...
          <Text style={s.paraIn}>{`(ბ) მოითხოვოს წილის შემძენისაგან სახელმწიფო რეგისტრაციისათვის საჭირო ყველა დოკუმენტის დროული გამოყოფა და ხელმოწერა;`}</Text>
          <Text style={s.paraIn}>{`(გ) მიიღოს სრული ინფორმაცია სახელმწიფო რეგისტრაციის პროცედურის მდგომარეობის შესახებ;`}</Text>
          <Text style={s.paraIn}>{`(დ) სასამართლოში სარჩელის წარდგენის გზით მოითხოვოს ვალდებულებების შესრულება ან/და ზიანის ანაზღაურება, თუ წილის შემძენი არ ასრულებს ხელშეკრულებით ნაკისრ ვალდებულებებს;`}</Text>
          ...
          <Text style={s.artTitle}>{`მუხლი ${art(5)}. წილის შემძენის ვალდებულებები`}</Text>
          <Text style={s.para}>{`${art(5)}.1. წილის შემძენი ვალდებულია:`}</Text>
          {!isGift && (
          ...
          <Text style={s.artTitle}>{`მუხლი ${art(6)}. წილის შემძენის უფლებები`}</Text>
          <Text style={s.para}>{`${art(6)}.1. სსიპ საჯარო რეესტრის ეროვნულ სააგენტოში ცვლილების სახელმწიფო რეგისტრაციის მომენტიდან წილის შემძენი იძენს პარტნიორის სრულ კანონიერ სტატუსს, მათ შორის:`}</Text>
          ...
          <Text style={s.para}>{`${art(6)}.2. წილის შემძენს უფლება აქვს სასამართლოში სარჩელის წარდგენის გზით მოითხოვოს ვალდებულებების შესრულება ან/და ზიანის ანაზღაურება, თუ წილის დამთმობი არ ასრულებს ხელშეკრულებით ნაკისრ ვალდებულებებს.`}</Text>
          ...
          <Text style={s.para}>{`${art(7)}.3. სახელმწიფო რეგისტრაციასთან დაკავშირებული ყველა ხარჯი — სახელმწიფო მოსაკრებლის ჩათვლით — ეკისრება წილის შემძენს, გარდა იმ შემთხვევისა, თუ მხარეებს სხვაგვარად შეუთანხმდებიათ.`}</Text>
          ...
          {!isGift && (
            <Text style={s.para}>{`${art(8)}.2. ნასყიდობის ფასის განსაზღვრულ ვადაში გადაუხდელობის შემთხვევაში წილის შემძენი იხდის ფულად საჯარიმო სანქციას — გადასახდელი თანხის 0.05 (ნულ მთელი ხუთი მეასედი) პროცენტს ყოველ ვადაგადაცილებულ კალენდარულ დღეზე, ვადაგადაცილებულ თანხაზე გაანგარიშებით.`}</Text>
          )}
          ...
          <Text style={s.para}>{`${art(11)}.1. წინამდებარე ხელშეკრულება შედგენილია ქართულ ენაზე, 2 (ორ) თანაბარი იურიდიული ძალის მქონე ეგზემპლარად, თითო — თითოეული მხარისათვის.`}</Text>
          <Text style={s.para}>{`${art(11)}.2. ხელშეკრულება ძალაში შედის ორივე მხარის მიერ ხელმოწერის მომენტიდან.`}</Text>
          <Text style={s.para}>{`${art(11)}.3. ხელშეკრულებაში ნებისმიერი ცვლილება ან დამატება ძალაშია მხოლოდ ორივე მხარის მიერ ხელმოწერილი წერილობითი შეთანხმების სახით.`}</Text>
